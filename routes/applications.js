...
function serializeApp(a) {
  return {
    ...
    school:          a.school,
    status:          a.status,
    schoolStatus:    a.schoolStatus || 'pending',
    jsmStatus:       a.jsmStatus || 'pending',
...
router.get('/', async (req, res) => {
  try {
    ...
    const apps = await Application.find(query).sort({ createdAt: -1 }).select('-details').lean();
...
router.patch('/:id', async (req, res) => {
  try {
    ...
    if (role === 'calon') {
      ...
      app.markModified('details'); // Required for Mongoose Mixed fields
      await app.save();
    ...
    // ── adminSchool: can approve or reject applications for their own school ─
    if (role === 'adminSchool') {
      const schoolAdmin = await User.findById(userId).lean();
      if (app.school !== schoolAdmin?.school)
        return res.status(403).json({ error: 'Forbidden: application is not from your school' });

      const { status, rejectionReason } = req.body;
      if (!['approved', 'rejected'].includes(status))
        return res.status(400).json({ error: 'status must be "approved" or "rejected"' });

      if (app.schoolStatus !== 'pending')
        return res.status(400).json({ error: 'Application has already been reviewed by school admin' });

      app.schoolStatus  = status;
      app.reviewedBy    = userId;
      app.reviewedAt    = new Date();
      // main status stays 'pending' until JSM makes the final decision

      // Notify all JSM admins of the school admin's decision
      const jsmAdmins = await User.find({ role: 'adminJSM' }).lean();

      if (status === 'approved') {
        for (const jsm of jsmAdmins) {
          await notifyUser(
            jsm._id,
            'Application Approved by School Admin',
            `${schoolAdmin.name} (${app.school}) has approved the application of ${app.applicantName} for ${app.position}.`
          );
        }
      }
      ...
      if (status === 'rejected') {
        ...
        for (const jsm of jsmAdmins) {
          await notifyUser(
            jsm._id,
            'Application Rejected by School Admin',
            `${schoolAdmin.name} (${app.school}) has rejected the application of ${app.applicantName} for ${app.position}.${rejectionReason ? ' Reason: ' + rejectionReason : ''}`
          );
        }
      }
