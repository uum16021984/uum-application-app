...
router.patch('/:id', async (req, res) => {
  try {
    ...
    if (role === 'calon' && isOwner) {
      const allowed = ['applicantName', 'position', 'status', 'details', 'qualification', 'experience', 'resume', 'coverLetter'];
      for (const key of allowed) {
        if (req.body[key] !== undefined) app[key] = req.body[key];
      }
      // Resolve grade and school from the job if jobId provided, else keep existing values
      if (req.body.jobId && mongoose.isValidObjectId(req.body.jobId)) {
        const linkedJob = await Job.findById(req.body.jobId).lean();
        if (linkedJob) {
          app.grade = linkedJob.grade;
          app.school = linkedJob.school;
          app.jobId = req.body.jobId;
          if (req.body.jobTitle) app.jobTitle = req.body.jobTitle;
        }
      } else if (req.body.grade !== undefined) {
        app.grade = req.body.grade;
      }
      await app.save();
