...
router.get('/', async (req, res) => {
  try {
    ...
    if (role === 'calon') {
      ...
      // School admin sees JSM-approved and JSM-rejected applications for their school
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // adminSchool can view approved and rejected apps for their school
    if (role === 'adminSchool') {
      const user = await User.findById(userId).lean();
      if (app.school !== user?.school || !['approved', 'rejected'].includes(app.status))
