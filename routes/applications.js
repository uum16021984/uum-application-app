...
router.get('/', async (req, res) => {
  try {
    ...
    if (role === 'calon') {
      ...
      // School admin only sees JSM-approved applications for their school
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // adminSchool can view only approved apps for their school
    if (role === 'adminSchool') {
      const user = await User.findById(userId).lean();
      if (app.school !== user?.school || app.status !== 'approved')
