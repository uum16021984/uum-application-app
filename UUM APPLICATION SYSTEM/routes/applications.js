      ...
      if (status != null) {
        ...
        if (status === 'approved' && app.applicantId) {
          await notifyUser(
            ...
            `Your application for ${app.position} has been approved by Admin JSM.`
          );
        }
        if (status === 'rejected' && app.applicantId) {
          await notifyUser(
            ...
            `We regret to inform you that your application for ${app.position} has been rejected.`
          );
        }
      }
    ...
    if (role === 'adminSchool' && adminUser && app.school === adminUser.school) {
      ...
      if (schoolApproved === true) {
        ...
        if (app.applicantId) {
          await notifyUser(
            app.applicantId,
            'Application Approved by School',
            `Your application for ${app.position} has been approved by ${app.school}.`
          );
        }
      }
      if (schoolRejected === true) {
        ...
        if (app.applicantId) {
          await notifyUser(
            app.applicantId,
            'Application Rejected by School',
            `Your application for ${app.position} has been rejected by ${app.school}. Reason: ${rejectionReason || 'N/A'}`
          );
        }
      }
