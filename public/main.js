...
async function submitFullApplication(isDraft = false) {
    ...
    try {
        ...
        // Include jobId so the server can resolve the correct school from the job record
    ...
    contentArea.innerHTML = schoolApps.map(app => `
        <div class="card">
            ...
            <div style="display:flex; gap:8px; margin-top:8px;">
                <button onclick="viewApplicationDetail('${app.id}')" class="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">
...
`).join("");
    ...
    contentArea.innerHTML = `
        ...
        function loadAdminSchoolDashboard() {
            ...
            const allSchoolApplications = applications.filter(a => a.school === currentUser.school);
            const schoolApplications = allSchoolApplications;
            ...
            contentArea.innerHTML = `
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div class="card text-center">
                            <div class="text-3xl font-bold primary-text">${allSchoolApplications.length}</div>
                            <div class="text-gray-600">Total Applications</div>
                        </div>
                        <div class="card text-center">
                            <div class="text-3xl font-bold text-green-600">${allSchoolApplications.filter(a => a.schoolApproved).length}</div>
                            <div class="text-gray-600">School Approved</div>
                        </div>
                        <div class="card text-center">
                            <div class="text-3xl font-bold text-red-600">${allSchoolApplications.filter(a => a.schoolRejected).length}</div>
      ...
      function viewApplicationDetail(applicationId) {
           ...
           if (currentUser.role === 'adminJSM' && application.status === 'pending') {
                ...
                `;
		   } else if (currentUser.role === 'adminSchool' && application.status === 'approved' && !application.schoolApproved && !application.schoolRejected) {
                actionButtons = `
