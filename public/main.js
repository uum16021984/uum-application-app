...
function exportAllApplicationsToExcel() {
    ...
    const rows = [];
    rows.push(['No.', 'Applicant Name', 'Position', 'Grade', 'School', 'Date Applied', 'Status']);
    ...
    const ws = XLSX.utils.aoa_to_sheet(rows);
    ws['!cols'] = [{ wch: 4 }, { wch: 24 }, { wch: 22 }, { wch: 8 }, { wch: 30 }, { wch: 14 }, { wch: 14 }];

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Pending Applications');

    const fileName = `Pending_Applications_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(wb, fileName);
