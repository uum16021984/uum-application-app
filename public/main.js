    ...
    const topActionBar = `
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:18px; flex-wrap:wrap; gap:10px;">
            ...
            <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                <span style="font-size:12px; font-weight:600; color:${app.schoolStatus==='approved'?'#28a745':app.schoolStatus==='rejected'?'#dc3545':'#f59e0b'}; background:#f3f4f6; padding:4px 10px; border-radius:4px; white-space:nowrap;">School: ${app.schoolStatus.toUpperCase()}</span>
                ${app.status === 'pending' ? `
                ...
                ` : `<span style="font-weight:600; font-size:13px; color:${app.status==='approved'?'#28a745':'#dc3545'};">JSM: ${app.status.toUpperCase()}</span>`}
...
async function loadEvaluatePage() {
    ...
    contentArea.innerHTML = `
        ...
        <div class="card mb-4">
            ...
            <p><strong>School's Decision:</strong> <span class="status-badge status-${app.schoolStatus}">${app.schoolStatus}</span></p>
...
// Collect every section (A, C–K) of the currently-open apply form into a plain
// object. Does not include uploaded files or passport photo — callers attach
// those separately since they involve async uploads. Shared by both a real
// submission and "Save My Info" (reusable saved info).
...
// Submit the full application (from calon side) — this always sends a real,
// pending (or status-preserving, if editing) application. Saving reusable
// info for later is a completely separate action — see saveDraft().
// ============================================================
async function submitFullApplication() {
    ...
    // Fetch existing application (if editing) first, so validation/merging can see previously uploaded files
    ...
    const existingTranscript = existingApp?.details?.uploadedFiles?.['transcriptCV']
        || currentUser.savedDraft?.uploadedFiles?.['transcriptCV'];
    if (!transcriptFile && !existingTranscript) {
    ...
    } else if (existingApp?.details?.passportPhoto) {
        formData.passportPhoto = existingApp.details.passportPhoto;
    } else if (currentUser.savedDraft?.passportPhoto) {
        // Reuse the passport photo from "Save My Info", if the form was
        // auto-filled from it and the applicant didn't upload a new one.
        formData.passportPhoto = currentUser.savedDraft.passportPhoto;
    }

    // Upload all edu file inputs to Cloudinary
    showToast('Uploading documents...', 'info');

    // Start with saved-draft files, then existing application files (which take
    // priority when editing), so previously uploaded/saved documents are kept
    // unless a new file is chosen for that specific field.
    const uploadedFiles = Object.assign(
        {},
        currentUser.savedDraft?.uploadedFiles || {},
        existingApp?.details?.uploadedFiles || {}
    );
...
// (duplicate submitFullApplication removed — use async submitFullApplication() above)
...
// register() — see auth.js
    ...
    menuItems.forEach(item => {
        ...
        li.innerHTML = `
            <a href="#" onclick="${item.action}(); closeSidebar(); return false;" 
        ...
        function toggleSidebar() {
            ...
            if (backdrop) backdrop.style.display = sidebar.classList.contains('sidebar-closed') ? 'none' : 'block';
        }

        // Close the sidebar (used by the backdrop tap, and after picking a menu item on mobile)
...
function selectJobAndApply(jobId) {
 ...
 if(currentUser.role === "adminJSM" || currentUser.role === "adminSchool"){
    ...
    // Auto-fill from previously saved info, if any — no extra click needed
    const banner = document.getElementById('savedDraftBanner');
    if (currentUser.savedDraft) {
        hydrateApplyFormFields(currentUser.savedDraft, currentUser.savedDraft.applicantName || currentUser.name);
        ...
        async function loadNotifications() {
            ...
            if (userNotifications.length === 0) {
                ...
                userNotifications.forEach(notification => {
                    ...
                    notificationsHTML += `
                        <div class="notification-item ${readClass}" onclick="dismissNotification('${notification.id}')" style="display:flex; justify-content:space-between; align-items:flex-start; gap:10px; cursor:pointer;">
                            <div>
                                <h3 class="font-semibold">${notification.title}</h3>
                                <p class="text-gray-600">${notification.message}</p>
                                <p class="text-sm text-gray-500 mt-2">${notification.date}</p>
                            </div>
                            <button onclick="event.stopPropagation(); deleteNotification('${notification.id}')" title="Delete notification" style="background:none; border:none; color:#dc2626; cursor:pointer; padding:4px 8px; flex-shrink:0;">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    `;
        ...
        // Clicking a notification marks it as read (like tapping a notification
        // in any other app) — it stays in the list, just no longer highlighted
        // as unread. Deleting is a separate, explicit action via the trash icon.
...
// Fill every section of the currently-open apply form from a details object.
// Shared by loadSavedDraftIntoForm() (reusable saved info) — the form must
// already be open and reset before calling this.
function hydrateApplyFormFields(d, applicantName) {
    d = d || {};
    ...
    if (applicantName) setVal('fullName', applicantName);
...
// Open the apply form pre-filled with an existing application's data — used
// by the "Edit" button on a submitted application, and by "Continue Draft"
// for any older draft-status Application records from before saved-info
// templates existed.
function editApplication(id) {
    const app = applications.find(a => String(a.id || a._id) === String(id));
    if (!app) {
    ...
    resetApplyForm();
    hydrateApplyFormFields(app.details || {}, app.applicantName);

    showToast("Application loaded — you can review and update it", "success");
}

// Manually (re)load the applicant's saved reusable info into the currently-open
// apply form. Runs automatically when the form first opens, but also exposed
// as a button so the applicant can reload it anytime — e.g. after editing
// fields, or after hitting "Clear & Start Blank" and changing their mind.
function loadSavedDraftIntoForm() {
    if (!currentUser.savedDraft) {
        showToast('No saved info found yet.', 'error');
        return;
    }
    hydrateApplyFormFields(currentUser.savedDraft, currentUser.savedDraft.applicantName || currentUser.name);
    showToast('Saved info loaded — review and adjust before submitting.', 'success');
}

// Clear the form back to blank, in case the auto-filled saved info doesn't
// apply to this application (e.g. very different job/situation).
function clearAutoFilledForm() {
    resetApplyForm();
    document.getElementById('fullName').value = currentUser.name || '';
    const banner = document.getElementById('savedDraftBanner');
    if (banner) banner.style.display = 'none';
                ...
                <div class="card">
                    <div class="border-b border-gray-200">
                        <nav class="-mb-px flex">
                            <button class="tab-btn py-2 px-4 border-b-2 font-medium text-sm" onclick="showTab('all-tab')">
                    ...
                    <div id="all-tab" class="tab-content active">
                        ${generateApplicationsTable(applications)}
                    </div>

                    <div id="ds11-tab" class="tab-content">
        ...
        function generateApplicationsTable(apps) {
            ...
            let tableHTML = `
                <div class="search-container" style="display:flex; gap:8px; align-items:center;">
                    <select class="search-category-select" onchange="filterByCategory(this)" style="padding:10px 12px; border:1px solid #d1d5db; border-radius:6px; font-size:14px; color:#374151; background:white; cursor:pointer;" title="Search within a specific category">
                        <option value="all">All Fields</option>
                        <option value="0">Name</option>
                        <option value="1">Grade</option>
                        <option value="2">School</option>
                        <option value="3">Date Applied</option>
                        <option value="4">JSM Decision</option>
                        <option value="5">School Decision</option>
                    </select>
                ...
                <div class="table-container">
                    <table>
                        <thead>
                            <tr>
                                <th><span class="th-label" onclick="sortTable(0)">Name <i class="fas fa-sort text-gray-400"></i></span></th>
                                <th><span class="th-label" onclick="sortTable(1)">Grade <i class="fas fa-sort text-gray-400"></i></span></th>
                                <th><span class="th-label" onclick="sortTable(2)">School <i class="fas fa-sort text-gray-400"></i></span></th>
                                <th>
                                    <span class="th-label" style="cursor:default;">
                                        <select class="year-filter-select" data-date-col="3" onchange="filterByYear(this)" title="Filter applications" style="font-size:12px; padding:3px 6px; border-radius:4px; border:1px solid #ccc; cursor:pointer; font-weight:600; color:var(--primary-color); background:#f7fafc;">
                                            <option value="all" selected>Date Applied</option>
                                            <option value="all-years">All Years</option>
                                            ${buildYearOptions(apps)}
                                        </select>
                                        <i class="fas fa-sort text-gray-400" onclick="sortTable(3)" style="cursor:pointer;" title="Sort by date applied"></i>
                                    </span>
                                </th>
                                <th><span class="th-label" onclick="sortTable(4)">JSM Decision <i class="fas fa-sort text-gray-400"></i></span></th>
                                <th><span class="th-label" onclick="sortTable(5)">School Decision <i class="fas fa-sort text-gray-400"></i></span></th>
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            apps.forEach(app => {
                tableHTML += `
                    <tr>
                        ...
                        <td><span class="status-badge status-${app.jsmStatus}">${app.jsmStatus}</span>${app.jsmStatus === 'rejected' && app.rejectionReason ? '<div style="font-size:0.75rem;color:#dc2626;margin-top:3px;">Reason: ' + app.rejectionReason + '</div>' : ''}</td>
                        <td><span class="status-badge status-${app.schoolStatus}">${app.schoolStatus}</span>${app.schoolStatus === 'rejected' && app.rejectionReason ? '<div style="font-size:0.75rem;color:#dc2626;margin-top:3px;">Reason: ' + app.rejectionReason + '</div>' : ''}</td>
                        <td>
...
`).join("");
    ...
    let rows = relevantApps.map(app => `
        <tr>
            ...
            <td>${app.school}</td>
            <td>${app.dateApplied}</td>
            <td><span class="status-badge status-${app.jsmStatus}">${app.jsmStatus}</span></td>
            <td><span class="status-badge status-${app.schoolStatus}">${app.schoolStatus}</span>${app.rejectionReason ? '<div style="font-size:0.75rem;color:#dc2626;margin-top:3px;">Reason: ' + app.rejectionReason + '</div>' : ''}</td>
            <td>
    ...
    contentArea.innerHTML = `
        ...
        <div class="card">
            ...
            <div class="table-container">
                <table>
                    <thead><tr><th>Nama</th><th>Jawatan</th><th>Gred</th><th>Sekolah</th><th>Tarikh</th><th>JSM Decision</th><th>School Decision</th><th>Action</th></tr></thead>
                    <tbody>${rows}</tbody>
                </table>
            </div>
        </div>`;
            ...
            visibleSchools.forEach(school => {
                ...
                const rows = schoolApps.map(app => {
                    return `<tr>
                        ...
                        <td style="padding:7px 10px;"><span class="status-badge status-${app.jsmStatus}">${app.jsmStatus}</span></td>
                        <td style="padding:7px 10px;">
                            <span class="status-badge status-${app.schoolStatus}">${app.schoolStatus}</span>
                    ...
                    <div class="card" style="margin-bottom:20px;">
                        <div style="background:#003087;color:white;padding:8px 14px;margin:-20px -20px 14px -20px;border-radius:8px 8px 0 0;display:flex;justify-content:space-between;align-items:center;">
                            ...
                            <span style="font-size:12px;opacity:0.85;">${schoolApps.length} application${schoolApps.length !== 1 ? 's' : ''} &nbsp;|&nbsp;
                                <span style="color:#fbbf24;">${schoolApps.filter(a=>a.schoolStatus==='pending').length} pending</span> &nbsp;·&nbsp;
                                <span style="color:#6ee7b7;">${schoolApps.filter(a=>a.schoolStatus==='approved').length} approved</span> &nbsp;·&nbsp;
                                <span style="color:#fca5a5;">${schoolApps.filter(a=>a.schoolStatus==='rejected').length} rejected</span>
                            </span>
                        </div>
                            ...
                            <table style="width:100%;border-collapse:collapse;font-size:12px;">
                                <thead>
                                    <tr style="background:#f0f4f8;text-align:left;">
                                        ...
                                        <th style="padding:7px 10px;">JSM Decision</th>
                                        <th style="padding:7px 10px;">School Decision</th>
        ...
        async function loadAdminSchoolDashboard() {
            ...
            // Pending/Approved/Rejected here reflect the SCHOOL's own decision
            // (schoolStatus), independent of JSM's separate decision (status).
            const pendingApps  = applications.filter(a => a.schoolStatus === 'pending');
            const approvedApps = applications.filter(a => a.schoolStatus === 'approved');
            const rejectedApps = applications.filter(a => a.schoolStatus === 'rejected');
                    ...
                    <div class="table-container">
                        <table>
                            <thead>
                                <tr>
                                    <th><span class="th-label" onclick="sortTable(0)">Name <i class="fas fa-sort text-gray-400"></i></span></th>
                                    <th><span class="th-label" onclick="sortTable(1)">Position <i class="fas fa-sort text-gray-400"></i></span></th>
                                    <th><span class="th-label" onclick="sortTable(2)">Grade <i class="fas fa-sort text-gray-400"></i></span></th>
                                    <th>
                                        <span class="th-label" style="cursor:default;">
                                            <select class="year-filter-select" data-date-col="3" onchange="filterByYear(this)" title="Filter applications" style="font-size:12px; padding:3px 6px; border-radius:4px; border:1px solid #ccc; cursor:pointer; font-weight:600; color:var(--primary-color); background:#f7fafc;">
                                                <option value="all" selected>Date Applied</option>
    ...
    document.getElementById('applicationDetailModal').classList.add('active');
        ...
        async function openSchoolViewForm(appId) {
            ...
            // Show BOTH decisions separately — School's own decision (schoolStatus)
            // and JSM's separate decision (jsmStatus) — since the two are
            // independent review tracks.
            const badgeFor = (s, label) => {
                const color = s === 'approved' ? '#28a745' : s === 'rejected' ? '#dc3545' : '#f59e0b';
                const bg = s === 'approved' ? '#e8f5e9' : s === 'rejected' ? '#fdecea' : '#fffbeb';
                const icon = s === 'approved' ? '✓' : s === 'rejected' ? '✗' : '⏳';
                return `<span style="font-weight:600; font-size:12px; color:${color}; background:${bg}; padding:4px 10px; border-radius:4px; white-space:nowrap;">${icon} ${label}: ${s.toUpperCase()}</span>`;
            };
            const statusBadge = `
                ${badgeFor(app.schoolStatus, "School")}
                ${badgeFor(app.jsmStatus, "JSM")}
                ${app.rejectionReason ? `<div style="font-size:12px; color:#dc3545; margin-top:4px; width:100%;">Reason: ${app.rejectionReason}</div>` : ''}
            `;

            // School can act as long as THEY haven't decided yet, independent of JSM
            const actionButtons = app.schoolStatus === 'pending' ? `
        ...
        // Filter applications
        // Applies both the text search box and the "Date Applied" year dropdown
        // together, scoped to whichever table they sit above (tab or card).
        function applyTableFilters(scope) {
            if (!scope) return;
            const table = scope.querySelector('table');
            if (!table) return;

            const searchInput = scope.querySelector('.search-input');
            const yearSelect = scope.querySelector('.year-filter-select');
            const categorySelect = scope.querySelector('.search-category-select');
            const searchTerm = searchInput ? searchInput.value.toUpperCase() : '';
            const yearValue = yearSelect ? yearSelect.value : 'all';
            const dateColIndex = yearSelect ? parseInt(yearSelect.dataset.dateCol, 10) : -1;
            const categoryColIndex = (categorySelect && categorySelect.value !== 'all')
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                if (searchTerm) {
                    if (categoryColIndex >= 0) {
                        // Search only within the chosen column
                        const cell = cells[categoryColIndex];
                        const textValue = cell ? (cell.textContent || cell.innerText) : '';
                        searchMatch = textValue.toUpperCase().indexOf(searchTerm) > -1;
                    } else {
                        // Search across all columns except the last (Action)
                        searchMatch = false;
                ...
                let yearMatch = true;
                if (yearValue !== 'all' && yearValue !== 'all-years' && dateColIndex >= 0 && cells[dateColIndex]) {
                    const cellDate = new Date(cells[dateColIndex].textContent || '');
                    yearMatch = !isNaN(cellDate) && String(cellDate.getFullYear()) === String(yearValue);
                }

                rows[i].style.display = (searchMatch && yearMatch) ? '' : 'none';
            }
        }

        function filterApplications(input, context) {
            const scope = input.closest('.tab-content') || input.closest('.card');
            applyTableFilters(scope);
        }

        // Filter a table by the column picked in the search category dropdown
        function filterByCategory(select) {
            const scope = select.closest('.tab-content') || select.closest('.card');
            applyTableFilters(scope);
        }

        // Filter a table by the year picked in the "Date Applied" column dropdown
        function filterByYear(select) {
            const scope = select.closest('.tab-content') || select.closest('.card');
            applyTableFilters(scope);
        }

        // Builds <option> tags for distinct years found in a list of applications' dateApplied field
        function buildYearOptions(apps) {
            const years = [...new Set(apps.map(a => {
                const d = new Date(a.dateApplied);
                return isNaN(d) ? null : d.getFullYear();
            }).filter(y => y !== null))].sort((a, b) => b - a);
...
let currentApplyJob = null; // simpan job yang sedang di-apply
        ...
        function showToast(message, type = 'info') {
            ...
            // Cancel any earlier pending hide-timer so a fast-following message
            // (e.g. "Uploading..." then "Submitted!") isn't cut short by a
            // leftover timeout from the previous toast.
        ...
        // ============================================================
// SAVE DRAFT FUNCTION ("Save My Info")
// ============================================================
// Saves everything currently filled in the form as the applicant's personal,
// reusable info — this does NOT create or submit an Application. It's purely
// so the applicant can reuse the same details next time they apply for any
// job (e.g. after a rejected application), instead of retyping everything.
async function saveDraft() {
    ...
    const details = collectFormDetails();
    details.applicantName = name;

    showToast('Saving your info...', 'info');

    try {
        // Preserve previously saved files if no new ones were chosen this time
        const prevSaved = currentUser.savedDraft || {};
        const uploadedFiles = Object.assign({}, prevSaved.uploadedFiles || {});

        const passportFile = document.getElementById('passportPhoto')?.files?.[0];
        if (passportFile) {
            const result = await uploadToCloudinary(passportFile);
            details.passportPhoto = result.url;
        } else if (prevSaved.passportPhoto) {
            details.passportPhoto = prevSaved.passportPhoto;
        }
        ...
        if (Object.keys(uploadedFiles).length > 0) details.uploadedFiles = uploadedFiles;

        const { user } = await api('/auth/me/draft', {
            method: 'PUT',
            body: JSON.stringify({ savedDraft: details }),
        });
        currentUser = user;
        showToast('Saved! You can reuse this info next time you apply for any job.', 'success');
    } catch (e) {
        showToast(e.message || 'Failed to save your info', 'error');
    }
}
...
async function loadSchoolEvaluatePage() {
    ...
    // School admin evaluates independently of JSM — filter by their OWN
    // decision (schoolStatus), not the shared/final status field (which only
    // JSM's decision sets). Otherwise, once JSM decides, the application would
    // wrongly disappear from the school's queue even if the school never
    // weighed in yet.
    const pendingApps = applications.filter(app => app.schoolStatus === 'pending');
    ...
    contentArea.innerHTML = `
        ...
        <div class="card mb-4">
            ...
            <p><strong>Grade:</strong> ${app.grade} &nbsp;|&nbsp; <strong>School:</strong> ${app.school}</p>
            <p><strong>Date Applied:</strong> ${app.dateApplied}</p>
            <p><strong>JSM's Decision:</strong> <span class="status-badge status-${app.jsmStatus}">${app.jsmStatus}</span></p>
            <div class="mt-3" style="display:flex; gap:8px; flex-wrap:wrap;">
