...
// register() — see auth.js
    ...
    jobOpenings.forEach(job => {
        ...
        jobOpeningsHTML += `
            <div class="card job-ad overflow-hidden p-0">
                ${job.image ? `<img src="${job.image}" alt="${job.title}" class="w-full object-cover" style="max-height:150px;">` : ''}
                <div class="p-4">
                <h3 class="text-lg font-semibold primary-text mb-2">${job.title}</h3>
                <p class="text-gray-600 mb-2">${job.description}</p>

                <div class="text-sm text-gray-500 mb-3">
                    <span>Grade: ${job.grade}</span><br>
                    <span>School: ${job.school}</span><br>
                    <span>Deadline: ${job.deadline}</span><br>
                    <span>Posted by: ${job.postedBy}</span>
                </div>

                <div class="mb-1 flex justify-between text-xs font-semibold text-gray-600">
                    <span>Vacancies</span>
                    <span class="${isFull ? 'text-red-600 font-bold' : spotsLeft === 1 ? 'text-yellow-600 font-bold' : 'text-green-700'}">
                        ${isFull ? 'Full &mdash; 0 spots left' : spotsLeft + ' spot' + (spotsLeft !== 1 ? 's' : '') + ' left of ' + vacancies}
                    </span>
                </div>
                ...
                <div class="flex justify-end">
                    ${currentUser.role === 'calon' && !isFull ? `
                        <button onclick="selectJobAndApply('${job.id}')"
                        class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                            <i class="fas fa-file-edit mr-1"></i> Apply Now
                        </button>
                    ` : isFull ? `<span class="text-red-600 font-bold text-sm"><i class="fas fa-ban mr-1"></i>No vacancies remaining</span>` : ``}
                </div>
                </div>
            </div>
        `;
            ...
            jobOpenings.forEach(job => {
                ...
                jobAdsHTML += `
                    <div class="card overflow-hidden p-0">
                        ${job.image ? `<img src="${job.image}" alt="${job.title}" class="w-full object-cover" style="max-height:160px;">` : ''}
                        <div class="p-4">
                            <div class="flex justify-between items-start mb-2">
                                <h3 class="text-lg font-semibold primary-text">${job.title}</h3>
                                <div class="flex gap-2 ml-2 shrink-0">
                                    <button onclick="editJobAd('${job.id}')" class="text-blue-500 hover:text-blue-700" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button onclick="deleteJobAd('${job.id}')" class="text-red-500 hover:text-red-700" title="Delete">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
                            </div>
                            <p class="text-gray-600 mb-2 text-sm">${job.description}</p>
                            <div class="text-sm text-gray-500 mb-3">
                                <span>Grade: ${job.grade}</span> &bull;
                                <span>School: ${job.school}</span> &bull;
                                <span>Deadline: ${job.deadline}</span>
                            </div>
                            <div class="mb-1 flex justify-between text-xs font-semibold">
                                <span>Vacancies filled</span>
                                <span class="${isFull ? 'text-red-600' : 'text-green-700'}">${approvedCount} / ${vacancies} &mdash; ${isFull ? '<span class="text-red-600">Full</span>' : spotsLeft + ' spot' + (spotsLeft !== 1 ? 's' : '') + ' left'}</span>
                            </div>
...
function createJobCard(job) {
    ...
    return `
        <div class="bg-white rounded shadow mb-4 overflow-hidden">
            ...
            <div class="mt-3 mb-1 flex justify-between text-xs font-semibold text-gray-600">
                <span>Vacancies</span>
                <span class="${isFull ? 'text-red-600 font-bold' : spotsLeft === 1 ? 'text-yellow-600 font-bold' : 'text-green-700'}">
                    ${isFull ? 'Full &mdash; 0 spots left' : spotsLeft + ' spot' + (spotsLeft !== 1 ? 's' : '') + ' left of ' + vacancies}
                </span>
            ...
            ${open && !isFull ?
                ...
                ` : ``)
            : isFull ?
                `<span class="inline-block mt-2 text-red-600 font-bold"><i class="fas fa-ban mr-1"></i>No vacancies remaining</span>`
            :
