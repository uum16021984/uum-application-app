...
async function loadSchoolAdminApplications() {
    ...
    if (applications.length === 0) {
        contentArea.innerHTML = `
            <div class="card">
                <p>No reviewed applications for your school yet.</p>
            </div>
        `;
        return;
    }
...
`).join("");
    ...
    contentArea.innerHTML = `
        ...
        async function loadAdminSchoolDashboard() {
            ...
            // applications array is server-filtered to approved+rejected for this school
            const approvedApps = applications.filter(a => a.status === 'approved');
            const rejectedApps = applications.filter(a => a.status === 'rejected');
            
            let applicationsHTML = '';
            if (applications.length === 0) {
                applicationsHTML = '<p class="text-gray-600">No reviewed applications for your school yet.</p>';
            } else {
                applicationsHTML = `
                    <div class="search-container">
