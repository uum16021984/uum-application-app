...
async function exportAllApplicationsToExcel() {
    ...
    for (const app of pendingApps) {
        try {
            fullApps.push(await api('/applications/' + app.id));
        } catch(e) {
            fullApps.push(app);
        }
    }
    ...
    const headers = [
        ...
        'Nama Penuh / Full Name', 'No. IC Baru / New I/C', 'No. IC Lama / Old I/C', 'Tarikh Lahir / DOB', 'Umur / Age', 'Agama / Religion',
        'No. Pasport / Passport No', 'Warganegara / Citizenship', 'Jantina / Gender', 'Telefon / Phone', 'Emel / Email',
        'Alamat Tetap / Permanent Address', 'Alamat Surat-menyurat / Mailing Address',
        'Taraf Perkahwinan / Marital Status', 'Bil. Anak / No. of Children', 'Nama Pasangan / Spouse Name', 'Nama Ibu Bapa / Parent Name',
        // Job info
        'Jawatan Dipohon / Position Applied', 'Gred / Grade', 'Bidang Pengkhususan / Specialization', 'Pusat Pengajian / School', 'Tarikh Permohonan / Date Applied',
        // Section C – Higher Education
        'PhD – Institusi / Institution', 'PhD – Tahun / Year', 'PhD – CGPA', 'PhD – Bidang / Field',
        'Master – Institusi / Institution', 'Master – Tahun / Year', 'Master – CGPA', 'Master – Bidang / Field',
        'Degree – Institusi / Institution', 'Degree – Tahun / Year', 'Degree – CGPA', 'Degree – Bidang / Field',
        'Diploma – Institusi / Institution', 'Diploma – Tahun / Year', 'Diploma – CGPA', 'Diploma – Bidang / Field',
        'SPM – Institusi / Institution', 'SPM – Tahun / Year', 'SPM – CGPA', 'SPM – Bidang / Field',
        // Section D – Professional Affiliation (first 2 rows)
        'Prof 1 – Nama Badan / Body', 'Prof 1 – Tarikh / Date', 'Prof 1 – No. Pendaftaran / Reg No',
        'Prof 2 – Nama Badan / Body', 'Prof 2 – Tarikh / Date', 'Prof 2 – No. Pendaftaran / Reg No',
        // Section E – Scholarship / Loan (first 2 rows)
        'Schol 1 – Badan / Body', 'Schol 1 – Tempoh / Duration', 'Schol 1 – Terikat / Bonded', 'Schol 1 – Jenis / Type',
        'Schol 2 – Badan / Body', 'Schol 2 – Tempoh / Duration', 'Schol 2 – Terikat / Bonded', 'Schol 2 – Jenis / Type',
        // Section F – Current Job
        'Jawatan Sekarang / Current Position', 'Majikan Sekarang / Current Employer', 'Gaji Sekarang / Current Salary', 'Tarikh Mula Kerja / Date Commenced',
        // Section G – Working Experience (first 2 rows)
        'Exp 1 – Jawatan / Position', 'Exp 1 – Majikan / Employer', 'Exp 1 – Dari / From', 'Exp 1 – Hingga / To',
        'Exp 2 – Jawatan / Position', 'Exp 2 – Majikan / Employer', 'Exp 2 – Dari / From', 'Exp 2 – Hingga / To',
        // Section H – Curricular & Social Activities (first row)
        'Kokurikulum – Peringkat Sekolah/Kolej/U/Komuniti', 'Kokurikulum – Peringkat Luar',
        // Section I – Publication (first 3)
        'Penerbitan 1 / Publication 1', 'Penerbitan 2 / Publication 2', 'Penerbitan 3 / Publication 3',
        // Section J – Research (first 3)
        'Penyelidikan 1 / Research 1', 'Penyelidikan 2 / Research 2', 'Penyelidikan 3 / Research 3',
        // Section K – Reference (first 2)
        'Rujukan 1 – Nama / Reference 1 Name', 'Rujukan 1 – Tel / Phone',
        'Rujukan 2 – Nama / Reference 2 Name', 'Rujukan 2 – Tel / Phone',
        // Scopus & CV
        'Scopus', 'Transkrip/CV',
        // Status
        'Status',
    ...
    fullApps.forEach((app, i) => {
        const d = app.details || {};
        const edu = (level) => {
            const e = (d.edu || []).find(x => x.level === level) || {};
            return [e.institution||'', e.year||'', e.cgpa||'', e.field||''];
        };
        const prof = (n) => {
            const p = (d.professional || [])[n] || {};
            return [p.body||'', p.date||'', p.regno||''];
        };
        const schol = (n) => {
            const s = (d.scholarship || [])[n] || {};
            return [s.body||'', s.duration||'', s.bonded||'', s.type||''];
        };
        const exp = (n) => {
            const w = (d.workExp || [])[n] || {};
            return [w.position||'', w.employer||'', w.from||'', w.to||''];
        };
        const cocurr = (n) => {
            const c = (d.cocurriculum || [])[n] || {};
            return [c.school||'', c.outside||''];
        };
        const pub = (n) => {
            const p = (d.publications || [])[n] || {};
            return p.title || p.author || '';
        };
        const research = (n) => {
            const r = (d.research || [])[n] || {};
            return r.title || r.field || '';
        };
        const ref = (n) => {
            const r = (d.references || [])[n] || {};
        ...
        rows.push([
            i + 1,
            // A
            app.applicantName||'', d.newIC||'', d.oldIC||'', d.dob||'', calcAgeFromDob(d.dob), d.religion||'',
            d.passportNo||'', d.citizenship||'', d.gender||'', d.phone||'', d.email||'',
            d.permAddress||'', d.mailAddress||'',
            d.maritalStatus||'', d.numChildren||'0', d.spouseName||'', d.parentName||'',
            // Job
            app.position||'', app.grade||'', d.specialization||'', app.school||'', app.dateApplied||'',
            // C
            ...edu('phd'), ...edu('master'), ...edu('degree'), ...edu('diploma'), ...edu('spm'),
            // D
            ...prof(0), ...prof(1),
            // E
            ...schol(0), ...schol(1),
            // F
            d.currentJob?.position||'', d.currentJob?.employer||'', d.currentJob?.salary||'', d.currentJob?.date||'',
            // G
            ...exp(0), ...exp(1),
            // H
            ...cocurr(0),
            ...
            ...ref(0), ...ref(1),
            // Files
            d.uploadedFiles?.['scopusUpload']?.url || d.uploadedFiles?.['scopusUpload']?.name || '',
            d.uploadedFiles?.['transcriptCV']?.url || d.uploadedFiles?.['transcriptCV']?.name || '',
            // Status
    ...
    // Auto-width columns
    ws['!cols'] = headers.map((h, i) => ({ wch: Math.max(h.length + 2, 16) }));
    ...
    showToast(`Exported ${fullApps.length} application(s) to Excel!`, 'success');
}
...
function calcAge() {
    ...
    if (!ageField || isNaN(dob)) { if (ageField) ageField.value = ''; return; }
           ...
           const application = applications.find(a => String(a.id || a._id) === String(applicationId));
            ...
            const detailHTML = `
                <div class="mb-4">
                    ...
                    <p><strong>Status:</strong> ${application.status && application.status !== 'pending' ? '<span class="status-badge status-approved">✓ Received</span>' : '<span class="status-badge status-pending">⏳ Not Received</span>'}</p>
                </div>
                ...
                <div class="flex justify-end">
                    ${actionButtons}
                    <button onclick="downloadApplicationPDF('${application.id}')" class="ml-2 bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition">
