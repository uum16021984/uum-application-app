    ...
    contentArea.innerHTML = `
        ...
        <div class="card" style="margin-top:12px;">
            ...
            <div style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:11px;">
                ...
                <tbody>
                    ${(d.edu || []).map(e => `<tr>
                        <td style="border:1px solid #ccc; padding:5px; font-weight:600; background:#fafafa;">${e.level?.toUpperCase() || '–'}</td>
                        ...
                        <td style="border:1px solid #ccc; padding:5px; text-align:center;">${d.uploadedFiles?.['edu_'+e.level+'_spm'] ? (d.uploadedFiles['edu_'+e.level+'_spm'].type?.startsWith('image/') ? '<img src="'+d.uploadedFiles['edu_'+e.level+'_spm'].data+'" style="max-width:80px; max-height:80px; object-fit:contain;">' : '<a href="'+d.uploadedFiles['edu_'+e.level+'_spm'].data+'" download="'+d.uploadedFiles['edu_'+e.level+'_spm'].name+'" style="color:#003087; font-size:11px;"><i class="fas fa-file-download"></i> Download</a>') : '–'}</td>
                        <td style="border:1px solid #ccc; padding:5px; text-align:center;">${d.uploadedFiles?.['edu_'+e.level+'_transcript'] ? (d.uploadedFiles['edu_'+e.level+'_transcript'].type?.startsWith('image/') ? '<img src="'+d.uploadedFiles['edu_'+e.level+'_transcript'].data+'" style="max-width:80px; max-height:80px; object-fit:contain;">' : '<a href="'+d.uploadedFiles['edu_'+e.level+'_transcript'].data+'" download="'+d.uploadedFiles['edu_'+e.level+'_transcript'].name+'" style="color:#003087; font-size:11px;"><i class="fas fa-file-download"></i> Download</a>') : '–'}</td>
                    </tr>`).join('')}
                </tbody>
            </table></div>
        </div>
...
async function submitFullApplication(isDraft = false) {
    ...
    if (passportFile) {
        formData.passportPhoto = await new Promise((resolve) => {
            const reader = new FileReader();
    ...
    // Collect per-education-level file uploads (cert + transcript) stored directly in edu rows
    for (const key of ['phd','master','degree','diploma']) {
        for (const type of ['spm','transcript']) {
            const input = document.getElementById(`edu_${key}_${type}`);
            if (input?.files?.[0]) {
                const file = input.files[0];
                const b64 = await new Promise(res => { const r = new FileReader(); r.onload = e => res(e.target.result); r.readAsDataURL(file); });
                const eduRow = formData.edu.find(e => e.level === key);
                if (eduRow) eduRow[type === 'spm' ? 'certFile' : type + 'File'] = { name: file.name, type: file.type, data: b64 };
            }
        }
    }

    // Collect all file inputs keyed by their ID so admin view can display them in the right section
    const allFileInputs = document.querySelectorAll('#contentArea input[type="file"]:not(#passportPhoto), #applyFormPage input[type="file"]:not(#passportPhoto)');
    const uploadedFiles = {};
    for (const input of allFileInputs) {
        if (input.files && input.files[0] && input.id) {
            ...
            uploadedFiles[input.id] = { name: file.name, type: file.type, data: base64 };
        }
    }
...
async function refreshAllData() {
    ...
    try {
        ...
        const fetches = [
            api('/applications'),
            api('/jobs'),
            isApplicant ? api('/notifications') : Promise.resolve([])
        ];
...
// register() — see auth.js
        ...
        function showDashboard() {
            ...
            // Show notification bell only for applicants
            const notifBtn = document.getElementById('notificationBtn');
            notifBtn.style.display = currentUser.role === 'calon' ? '' : 'none';
    ...
    userApplications.forEach(app => {
        ...
        <tr class="border-t">
            ...
            <td class="p-2">
                ...
                ${app.status === 'rejected' && app.rejectionReason ? `<div style="font-size:0.78rem;color:#dc2626;margin-top:4px;max-width:180px;">Reason: ${app.rejectionReason}</div>` : ''}
            </td>
...
`).join("");
    ...
    contentArea.innerHTML = `
      ...
      function viewApplicationDetail(applicationId) {
        ...
        // openSchoolViewForm — read-only view for adminSchool (View + Download only)
        function openSchoolViewForm(appId) {
            const app = applications.find(a => String(a.id) === String(appId));
            if (!app) { showToast('Application not found', 'error'); return; }
            ...
            const statusBadge = app.status === 'approved'
                ? `<span style="font-weight:600; font-size:13px; color:#28a745; background:#e8f5e9; padding:4px 10px; border-radius:4px;">✓ JSM APPROVED</span>`
                : `<span style="font-weight:600; font-size:13px; color:#dc3545; background:#fdecea; padding:4px 10px; border-radius:4px;">✗ JSM REJECTED${app.rejectionReason ? ': ' + app.rejectionReason : ''}</span>`;

            const downloadBtn = app.status === 'approved'
                ? `<button onclick="downloadApplicationWord('${app.id}')" style="background:#17a2b8; color:white; border:none; padding:8px 16px; border-radius:5px; cursor:pointer; font-size:13px; font-weight:600;">
                       <i class="fas fa-file-word mr-1"></i> Download Word
                   </button>`
                : '';
