        ...
        async function loadAdminSchoolDashboard() {
            ...
            if (applications.length === 0) {
                ...
                applications.forEach(app => {
                    const statusClass = `status-${app.schoolStatus}`;
                    const finalDiffers = app.status !== app.schoolStatus;
                    applicationsHTML += `
                        <tr>
                            <td>${app.applicantName}</td>
                            <td>${app.position}</td>
                            <td>${app.grade}</td>
                            <td>${app.dateApplied}</td>
                            <td><span class="status-badge ${statusClass}">${app.schoolStatus}</span>${finalDiffers ? '<div style="font-size:0.7rem;color:#6b7280;margin-top:3px;">Final (JSM): ' + app.status + '</div>' : ''}${app.rejectionReason ? '<div style="font-size:0.75rem;color:#dc2626;margin-top:3px;">Reason: ' + app.rejectionReason + '</div>' : ''}</td>
                            <td>
