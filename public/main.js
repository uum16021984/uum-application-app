...
function selectJobAndApply(jobId) {
 ...
 if(currentUser.role === "adminJSM" || currentUser.role === "adminSchool"){
    ...
    // Offer to reuse previously saved info, if any
    const banner = document.getElementById('savedDraftBanner');
    if (banner) banner.style.display = currentUser.savedDraft ? 'flex' : 'none';
}
...
// Load the applicant's saved reusable info (from a previous "Save My Info" click)
// into the currently-open, blank application form. Does NOT touch any
// Application record — this is purely personal data the applicant chose to reuse.
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <span onclick="sortTable(3)" style="cursor:pointer;">Date Applied <i class="fas fa-sort text-gray-400"></i></span>
                                    <select class="year-filter-select" data-date-col="3" onchange="filterByYear(this)" title="Filter by year applied" style="margin-left:6px; font-size:11px; padding:1px 3px; border-radius:4px; border:1px solid #ccc; cursor:pointer; font-weight:normal;">
    ...
    document.getElementById('applicationDetailModal').classList.add('active');
        ...
        function applyTableFilters(scope) {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                let yearMatch = true;
                if (yearValue !== 'all' && dateColIndex >= 0 && cells[dateColIndex]) {
