...
async function downloadApplicationExcel(appId) {
    ...
    addRow('Scopus', scopusFile ? (scopusFile.url || scopusFile.name || 'Uploaded') : 'Not uploaded');
    addBlank();
    ...
    addHeader('STATUS');
    addRow('Status', (app.status && app.status !== 'pending') ? 'Received' : 'Not Received');
...
async function exportAllApplicationsToExcel() {
    ...
    for (let i = 0; i < pendingApps.length; i++) {
        let app;
        try {
            app = await api('/applications/' + pendingApps[i].id);
        } catch(e) {
            app = pendingApps[i];
        }
        const d = app.details || {};
        ...
        const rows = [];
        const h = (text) => rows.push([text]);
        const r = (...cells) => rows.push(cells);
        const b = () => rows.push([]);

        h('BORANG PERMOHONAN JAWATAN AKADEMIK / APPLICATION FORM FOR ACADEMIC POST');
        b();
        r('JAWATAN / Position', app.position || '');
        r('GRED / Grade', app.grade || '');
        r('BIDANG PENGKHUSUSAN / Specialization', d.specialization || '');
        r('PUSAT PENGAJIAN / School', app.school || '');
        r('TARIKH PERMOHONAN / Date Applied', app.dateApplied || '');
        b();

        h('(A) BUTIRAN PERIBADI / PERSONAL DETAILS');
        r('Nama Penuh / Full Name', app.applicantName || '');
        r('No. K/P Baru / New IC', d.newIC || '');
        r('No. K/P Lama / Old IC', d.oldIC || '');
        r('Tarikh Lahir / Date of Birth', d.dob || '');
        r('Umur / Age', d.age || '');
        r('Agama / Religion', d.religion || '');
        r('No. Pasport / Passport No', d.passportNo || '');
        r('Kewarganegaraan / Citizenship', d.citizenship || '');
        r('Jantina / Gender', d.gender || '');
        r('Telefon / Phone', d.phone || '');
        r('Emel / Email', d.email || '');
        r('Alamat Tetap / Permanent Address', d.permAddress || '');
        r('Alamat Surat-menyurat / Mailing Address', d.mailAddress || '');
        b();

        h('(C) MAKLUMAT PENGAJIAN TINGGI / HIGHER EDUCATION DETAILS');
        r('Tahap / Level', 'Institusi / Institution', 'Tahun Tamat / Year', 'CGPA', 'Bidang / Field');
        (d.edu || []).forEach(e => r((e.level||'').toUpperCase(), e.institution||'', e.year||'', e.cgpa||'', e.field||''));
        b();

        h('(D) PENDAFTARAN PROFESIONAL / PROFESSIONAL AFFILIATION');
        r('Nama Badan', 'Tarikh Keahlian', 'No. Pendaftaran');
        (d.professional || []).forEach(p => r(p.body||'', p.date||'', p.regno||''));
        b();

        h('(E) BIASISWA / SCHOLARSHIP & LOAN');
        r('Badan yang Memberi', 'Tempoh', 'Terikat/Tidak', 'Jenis');
        (d.scholarship || []).forEach(s => r(s.body||'', s.period||'', s.bound||'', s.type||''));
        b();

        h('(F) PEKERJAAN SEKARANG / CURRENT JOB');
        r('Jawatan', d.currentPos || '');
        r('Majikan', d.currentEmp || '');
        r('Gaji & Gred', d.currentSalary || '');
        r('Tarikh Mula', d.currentFrom || '');
        b();

        h('(G) PENGALAMAN KERJA / WORKING EXPERIENCE');
        r('Jawatan', 'Majikan', 'Gaji', 'Dari', 'Hingga', 'Sebab Berhenti');
        (d.workExp || []).filter(w => w.position || w.employer).forEach(w =>
            r(w.position||'', w.employer||'', w.salary||'', w.from||'', w.to||'', w.reason||''));
        b();

        h('(H) KOKURIKULUM & SOSIAL / CO-CURRICULAR & SOCIAL ACTIVITIES');
        r('Peringkat Sekolah/Universiti', 'Peringkat Luar');
        (d.cocurriculum || [{school: d.cocurriculumSchool||'', outside: d.cocurriculumOutside||''}])
            .forEach(c => r(c.school||'', c.outside||''));
        b();

        h('(I) PENERBITAN / PUBLICATION');
        r('Tajuk / Title', 'Nama Jurnal', 'Tahun', 'Pengarang / Author');
        (d.publications || []).forEach(p => r(p.title||'', p.journal||'', p.year||'', p.author||''));
        b();

        h('SCOPUS');
        const scopusFile = d.uploadedFiles?.['scopusUpload'];
        r('Scopus Document', scopusFile ? (scopusFile.url || scopusFile.name || 'Uploaded') : 'Not uploaded');
        b();

        h('(J) PENYELIDIKAN / RESEARCH');
        r('Tajuk / Title', 'Bidang / Field');
        (d.research || []).forEach(res => r(res.title||'', res.field||''));
        b();

        h('TRANSKRIP / CV');
        const cvFile = d.uploadedFiles?.['transcriptCV'];
        r('Transcript/CV', cvFile ? (cvFile.url || cvFile.name || 'Uploaded') : 'Not uploaded');
        b();

        h('STATUS');
        r('Status', (app.status && app.status !== 'pending') ? 'Received' : 'Not Received');
        b();

        const ws = XLSX.utils.aoa_to_sheet(rows);
        ws['!cols'] = [{ wch: 36 }, { wch: 32 }, { wch: 20 }, { wch: 14 }, { wch: 28 }, { wch: 20 }];

        // Sheet name: applicant name truncated to 31 chars (Excel limit)
        const sheetName = (app.applicantName || `Applicant ${i+1}`).replace(/[:\\\/?*\[\]]/g, '').substring(0, 31);
        XLSX.utils.book_append_sheet(wb, ws, sheetName);
    }

    const fileName = `Applications_${new Date().toISOString().split('T')[0]}.xlsx`;
    XLSX.writeFile(wb, fileName);
    showToast(`Exported ${pendingApps.length} application(s) to Excel!`, 'success');
}
