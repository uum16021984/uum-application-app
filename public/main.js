                ...
                <div class="card-body">
                    ...
                    <div style="display:flex;justify-content:flex-end;">
                        ${(() => {
                            ...
                            if (isFull) return `<span style="color:#dc2626;font-weight:700;font-size:0.85rem;"><i class="fas fa-ban" style="margin-right:4px;"></i>No vacancies remaining</span>`;
                            if (!isJobOpen(job)) return `<span style="color:#6b7280;font-weight:700;font-size:0.85rem;"><i class="fas fa-clock" style="margin-right:4px;"></i>Application Closed</span>`;
                ...
                jobAdsHTML += `
                    <div class="card-flush">
                        ...
                        <div class="card-body">
                            ...
                            <p style="color:#4b5563;font-size:0.875rem;margin-bottom:8px;"><strong>Area of Expertise:</strong> ${job.description}</p>
                            <p style="font-size:0.8rem;color:#6b7280;margin-bottom:12px;">Grade: ${job.grade} &bull; School: ${job.school} &bull; Vacancies: ${job.vacancies ?? 1} (${job.approvedCount ?? 0} approved) &bull; Deadline: ${job.deadline}</p>
    ...
    return `
        <div class="card-flush">
            ...
            <div class="card-body">
                ...
                <p style="font-size:0.85rem;color:#374151;margin-bottom:2px;"><b>Vacancies:</b> ${job.vacancies ?? 1}</p>
                ...
                ${open && !isFull ?
                    (currentUser.role === 'calon' ? (() => {
                        const myApp = getMyApplicationForJob(job.id);
                        if (myApp && myApp.status !== 'draft') {
                            return `<span style="color:#15803d;font-weight:700;font-size:0.875rem;background:#dcfce7;padding:8px 16px;border-radius:6px;display:inline-block;"><i class="fas fa-check-circle" style="margin-right:4px;"></i> Applied</span>`;
                        }
                        if (myApp && myApp.status === 'draft') {
                            return `<button onclick="editApplication('${myApp.id}')" style="background:#f59e0b;color:white;padding:8px 16px;border:none;border-radius:6px;cursor:pointer;font-size:0.875rem;">
                                <i class="fas fa-pen" style="margin-right:4px;"></i> Continue Draft
...
`).join("");
    ...
    const row = (label, value) => `
        <div style="display:flex; gap:10px; padding:5px 0; border-bottom:1px solid #f0f0f0;">
            <div style="min-width:180px; font-size:12px; font-weight:600; color:#374151;">${label}</div>
            <div style="font-size:12px; color:#111;">${value || '–'}</div>
        </div>`;

    const sectionHeader = (title) => `
        <div style="background:#003087; color:white; padding:6px 12px; margin:14px -20px 10px -20px; font-size:12px; font-weight:600;">${title}</div>`;

    const detailHTML = `
        <div style="max-height:65vh; overflow-y:auto; padding:0 4px;">

            ${sectionHeader('MAKLUMAT JAWATAN / JOB APPLIED')}
            ${row('Jawatan / Position', application.position)}
            ${row('Gred / Grade', application.grade)}
            ${row('Pusat Pengajian / School', application.school)}
            ${row('Bidang Pengkhususan', d.specialization)}
            ${row('Tarikh Permohonan', application.dateApplied)}

            ${sectionHeader('(A) BUTIRAN PERIBADI / PERSONAL DETAILS')}
            ${row('Nama Penuh', application.applicantName)}
            ${row('No. K/P Baru', d.newIC)}
            ${row('No. K/P Lama', d.oldIC)}
            ${row('Tarikh Lahir', d.dob)}
            ${row('Umur / Age', d.age)}
            ${row('Agama / Religion', d.religion)}
            ${row('Jantina / Gender', d.gender)}
            ${row('Kewarganegaraan', d.citizenship)}
            ${row('Telefon / Phone', d.phone)}
            ${row('Emel / Email', d.email)}
            ${row('Alamat Tetap', d.permAddress)}
            ${row('Alamat Surat-menyurat', d.mailAddress)}

            ${sectionHeader('(C) MAKLUMAT PENGAJIAN TINGGI / HIGHER EDUCATION')}
            <table style="width:100%; border-collapse:collapse; font-size:11px; margin-top:4px;">
                <thead><tr style="background:#f0f4f8;">
                    <th style="padding:5px 8px; border:1px solid #ccc; text-align:left;">Tahap</th>
                    <th style="padding:5px 8px; border:1px solid #ccc; text-align:left;">Institusi</th>
                    <th style="padding:5px 8px; border:1px solid #ccc; text-align:left;">Tahun</th>
                    <th style="padding:5px 8px; border:1px solid #ccc; text-align:left;">CGPA</th>
                    <th style="padding:5px 8px; border:1px solid #ccc; text-align:left;">Bidang</th>
                </tr></thead>
                <tbody>
                    ${(d.edu || []).map(e => `<tr>
                        <td style="padding:4px 8px; border:1px solid #ccc; font-weight:600;">${(e.level||'').toUpperCase()}</td>
                        <td style="padding:4px 8px; border:1px solid #ccc;">${e.institution||'–'}</td>
                        <td style="padding:4px 8px; border:1px solid #ccc;">${e.year||'–'}</td>
                        <td style="padding:4px 8px; border:1px solid #ccc;">${e.cgpa||'–'}</td>
                        <td style="padding:4px 8px; border:1px solid #ccc;">${e.field||'–'}</td>
                    </tr>`).join('')}
                </tbody>
            </table>

            ${sectionHeader('(F) PEKERJAAN SEKARANG / CURRENT JOB')}
            ${row('Jawatan', (d.currentJob||{}).position || d.currentPos)}
            ${row('Majikan', (d.currentJob||{}).employer || d.currentEmp)}
            ${row('Gaji & Gred', (d.currentJob||{}).salary || d.currentSalary)}

            ${sectionHeader('(G) PENGALAMAN KERJA / WORK EXPERIENCE')}
            ${(d.workExp||[]).filter(w=>w.position||w.employer).length === 0 ? '<p style="font-size:12px;color:#999;">–</p>' :
              (d.workExp||[]).filter(w=>w.position||w.employer).map(w =>
                row(w.position||'–', `${w.employer||''} ${w.from ? '(' + w.from + ' – ' + (w.to||'present') + ')' : ''}`)
              ).join('')}

            ${sectionHeader('(I) PENERBITAN / PUBLICATION')}
            ${(d.publications||[]).filter(p=>p.title||p.author).length === 0 ? '<p style="font-size:12px;color:#999;">–</p>' :
              (d.publications||[]).filter(p=>p.title||p.author).map(p => row(p.title||'', p.author||'')).join('')}

            ${sectionHeader('STATUS')}
            ${row('Status', application.status && application.status !== 'pending'
                ? '<span class="status-badge status-approved">✓ Received</span>'
                : '<span class="status-badge status-pending">⏳ Not Received</span>')}
        </div>
...
async function saveDraft() {
    ...
    try {
        ...
        // IMPORTANT: keep currentApplyJob set and switch into "editing" mode for the
        // draft we just created. Without this, a second click on Save Draft / Submit
        // Application (on the same still-open form) had no jobId or edit-id to send,
        // which the server rejected with "Valid jobId is required".
        currentEditApplicationId = created.id;
        loadMyApplications();
    } catch (e) {
