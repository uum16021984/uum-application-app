...
// ── Toggle Full / Available ─────────────────────────────────────────────────

async function toggleJobFull(jobId, makeFull) {
  try {
    await api('/jobs/' + jobId, { method: 'PATCH', body: JSON.stringify({ isFull: makeFull }) });
    showToast(makeFull ? 'Marked as Unavailable — applicants can still see it, but can no longer apply' : 'Marked as Available', 'success');
    await refreshAllData();
    loadJobAds();
  } catch (e) {
