...
async function saveJobAd() {
  ...
  // Get image — new file takes priority, otherwise keep existing (for edit)
  let image = await getJobImageBase64();
  if (image === null && editId) {
    // No new file chosen — preserve the existing image
...
// kept for backwards-compat in case anything still calls createJobAd()
